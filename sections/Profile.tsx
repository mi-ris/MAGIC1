...
const Profile: React.FC<Props> = ({ user, onBack, onLogout, onUpdate }) => {
  ...
  const handleUpdate = (data: Partial<User>) => {
    const usersData = localStorage.getItem('magictris_database');
    if (usersData) {
  ...
  const handleNicknameBlur = () => {
    if (tempNickname.trim() !== user.nickname) {
        handleUpdate({ nickname: tempNickname.trim() });
    }
  };
  ...
  return (
    <div className="p-4 pt-16 max-w-4xl mx-auto space-y-4 pb-12 relative">
      ...
      {/* Tarjeta de Perfil ensanchada y compacta verticalmente */}
      <div className="bg-white/90 backdrop-blur-xl rounded-[3rem] shadow-2xl overflow-hidden border-[8px] border-white ring-4 ring-blue-100/30">
        ...
        {/* Cabecera con Avatar - Altura reducida */}
        ...
        <div className="p-6 sm:p-8 space-y-6 bg-white/50">
          ...
          {/* Fila de Datos Personales (Larga y Amplia) - Padding reducido */}
          ...
          {/* Fila del Apodo Editable - Padding reducido */}
          <div className="space-y-1">
          ...
          {/* Estadísticas y Avatares en una fila compartida si hay espacio, o simplemente más pequeños */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* Selección de Avatar - Más compacto */}
      ...
      {/* MODAL DE CONFIRMACIÓN CON REBOTE */}
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-indigo-950/50 backdrop-blur-lg animate-fade-in">
          <div className="bg-white/95 backdrop-blur-2xl border-[10px] border-blue-400 p-8 rounded-[4rem] shadow-2xl w-full max-w-md flex flex-col items-center text-center transform animate-[bounceIn_0.6s_ease-out]">
            ...
            <h3 className="text-3xl font-magic text-blue-800 mb-2 leading-tight">¿SEGURO QUE QUIERES SALIR?</h3>
            <p className="text-lg font-bold text-gray-500 mb-6">¡Gumi y las letras te esperarán!</p>
            
            <div className="w-full space-y-3">
              <button 
                onClick={onLogout}
                className="btn-magic-pop w-full bg-red-500 text-white py-4 rounded-[2rem] text-2xl font-magic shadow-xl border-b-6 border-red-700 active:translate-y-1"
              >
                CONFIRMAR ✅
              </button>
              ...
              <button 
                ...
                className="w-full bg-blue-100 text-blue-600 py-4 rounded-[2rem] text-xl font-magic border-2 border-blue-200 hover:bg-blue-200 transition-all active:scale-95"
              >
                CANCELAR ❌
              </button>
            </div>
